...
<head>
    ...
    <style>
        ...
        * { 
        ...
        /* KUNCI LEBAR HALAMAN DI SINI AGAR TIDAK MENTOK KANAN-KIRI */
        .pdf-wrapper {
            width: 180mm; /* Mengunci lebar konten statis di dalam standar cetak A4 */
            margin: 15mm auto; /* Memberikan jarak atas-bawah dan otomatis rata tengah */
        }
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">{{ Carbon::parse($d->bm_tanggal)->translatedFormat('d/m/Y') }}</td>
                    <td class="text-center">{{ $d->bm_kode }}</td>
                    <td class="text-center">{{ $d->barang_kode }}</td>
                    <td class="text-left">{{ $d->customer_nama ?? '-' }}</td>
                    <td class="text-left">{{ $d->barang_nama ?? 'Barang Dihapus' }}</td>
                    <td class="text-center">{{ $d->bm_jumlah }}</td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" style="text-align:center; color:#555; padding: 15px; font-style: italic;">
                        Tidak ada data barang masuk yang ditemukan.
                    </td>
                </tr>
                @endforelse
            </tbody>
